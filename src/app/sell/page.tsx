          ...
          {saleResult && (
            <div className="bg-emerald-50/50 border border-emerald-300 rounded-3xl p-6 space-y-4 shadow-xl animate-in fade-in slide-in-from-bottom-3 duration-300">
              ...
              {(saleResult.patientName || saleResult.doctorName) && (
                <div className="grid grid-cols-2 gap-3 text-xs bg-white p-3.5 rounded-2xl border border-emerald-200/60 font-medium">
                  {saleResult.patientName && (
                    <div>
                      <span className="text-slate-400 text-[10px] block font-bold uppercase">Patient</span>
                      <span className="font-extrabold text-slate-800 text-xs">{saleResult.patientName}</span>
                    </div>
                  )}
                  {saleResult.doctorName && (
                    <div>
                      <span className="text-slate-400 text-[10px] block font-bold uppercase">Doctor</span>
                      <span className="font-extrabold text-slate-800 text-xs">{saleResult.doctorName}</span>
                    </div>
                  )}
                </div>
              )}
